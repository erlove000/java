...
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_prod as cd where pkwsid='1910B54247' and isconmig is null and  new_sewerage_application_number is null and new_water_application_number is null""".format(table_name, batch)
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_prod as cd where batchname='1' """.format(table_name, batch)
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_prod as cd where pkwsid='1910MS50779' and isconmig is null """.format(table_name, batch)
postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_prod as cd where conn_type='B' and isconmig is null and water_upload_status is null and sewerage_upload_status is null and ownername is not null and propertyno is not null offset 10 limit 10""".format(table_name, batch)
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_prod as cd where conn_type='S' and isconmig is null and  sewerage_upload_status is null and ownername is not null and propertyno is not null""".format(table_name, batch)
...
def main():
    ...
    while continue_processing:
        ...
        for row in data:
            ...
            elif type=="W" or type == "WATER" or type == "W ":
                if iswater !='COMPLETED':
                    print("Connection Is of Water Type")
                    WaterConnection(property_response, json_data)

            elif type == "S" or type == "SEWRAGE" or type == "S ":
                if issewerage !='COMPLETED':
                    print("Connection Is of Sewerage Type")
                    SewerageConnection(property_response, json_data)

            conn_query = "Select row_to_json(cd) from patiala_migrate_records_prod as cd where pkwsid='" + connectionumber + "'"
            ...
            for row in data:
                ...
                iswater = json_data["water_upload_status"]
                issewerage =json_data["sewerage_upload_status"]
            if ((type == "B" or type == "B ") and iswater == "COMPLETED" and issewerage=="COMPLETED"):
                update_query_seweragetable = "UPDATE patiala_migrate_records_prod SET isconmig='True' WHERE pkwsid='" + connectionumber + "';"
            ...
            elif ((type == "WATER" or type == "W") and iswater == "COMPLETED"):
                update_query_seweragetable = "UPDATE patiala_migrate_records_prod SET isconmig='True' WHERE pkwsid='" + connectionumber + "';"
                cursor_ws.execute(update_query_seweragetable)
                connection_db_ws.commit()
                print("Migrated Successfully of Connection Type " + type + " Having  Consumer Code " + connectionumber)
            elif ((type == "S " or type == "S") and issewerage=="COMPLETED" ):
                update_query_seweragetable = "UPDATE patiala_migrate_records_prod SET isconmig='True' WHERE pkwsid='" + connectionumber + "';"
                cursor_ws.execute(update_query_seweragetable)
                connection_db_ws.commit()
                print("Migrated Successfully of Connection Type " + type + " Having  Consumer Code " + connectionumber)
