...
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_uat as cd where pkwsid='1910B54247' and isconmig is null and  new_sewerage_application_number is null and new_water_application_number is null""".format(table_name, batch)
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_uat as cd where batchname='1' """.format(table_name, batch)
postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_uat as cd where pkwsid='1910MB77134' """.format(table_name, batch)
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_uat as cd where conn_type='B' and isconmig is null and water_upload_status is null and sewerage_upload_status is null and ownername is not null and propertyno is not null""".format(table_name, batch)
# postgresql_select_Query = """select row_to_json(cd) from patiala_migrate_records_uat as cd where conn_type='S' and isconmig is null and  sewerage_upload_status is null and ownername is not null and propertyno is not null""".format(table_name, batch)
...
def main():
    ...
    while continue_processing:
        ...
        for row in data:
            ...
            conn_query = "Select row_to_json(cd) from patiala_migrate_records_uat as cd where pkwsid='" + connectionumber + "'"
            ...
            if ((type == "B" or type == "B ") and iswater == "COMPLETED" and issewerage=="COMPLETED"):
                update_query_seweragetable = "UPDATE patiala_migrate_records_uat SET isconmig='True' WHERE pkwsid='" + connectionumber + "';"
