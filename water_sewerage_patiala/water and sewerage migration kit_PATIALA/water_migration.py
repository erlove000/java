from dbconfig import *
import uuid
from decimal import Decimal, ROUND_HALF_UP
from datetime import datetime






class WaterConnection:
    def __init__(self, property_reponse, water_data):
        self.property_reponse = property_reponse
        self.water_data = water_data
        self.createwater(self)

    def createwater(self, waterData):
        print("Creating Water Application")
        connType = waterData.water_data['conn_type']
        if connType.strip() == "B":
            iswaterconn = True
            issewerageconn = True
            isname = "Water And Sewerage"
        elif connType.strip().upper() == "W":
            iswaterconn = True
            issewerageconn = False
            isname = "Water"
        url = "https://mseva.lgpunjab.gov.in/ws-services/wc/_create"
        if (waterData.water_data['mobile'] is None or waterData.water_data['mobile'] == ''):
            waterData.water_data['mobile'] = '9999999999'
        else:
            waterData.water_data['mobile'] = waterData.water_data['mobile']

        oldconnectionno = waterData.water_data['oldconnectionno']

        cursor_ws.execute("""
            SELECT ap.previous_water,
                   ap.previous_sewerage,
                   ap.total_previous,
                   ap.current_water,
                   ap.current_sewerage,
                   ap.surcharge,
                   ap.net_amt,
                   ap.gross_amt,
                   ap.actual_total_arrrear_included_surcharge
            FROM arrear_pending ap
            LEFT JOIN patiala_migrate_records_prod pmru 
                   ON ap.id_no = pmru.oldconnectionno
            WHERE ap.id_no = %s
        """, (oldconnectionno,))

        db_result = cursor_ws.fetchone()

        # Assign default values if no record found
        previous_water = current_water = total_previous = previous_sewerage = current_sewerage = surcharge = net_amt = gross_amt = actual_total_arrrear_included_surcharge = None

        if db_result:
            (previous_water, previous_sewerage, total_previous, current_water, current_sewerage, surcharge, net_amt, gross_amt, actual_total_arrrear_included_surcharge) = db_result

        conn_holder_detail = conn_holder_details()
        conn_holder = conn_holder_detail.ownerdetails(waterData.water_data['ownername'], waterData.water_data['mobile'])
        ConnHolderDetail = json.loads(conn_holder)
        headers = {
            "Content-Type": "application/json",
        }
        request_body = {
            'RequestInfo': {
                'apiId': 'Rainmaker',
                'ver': '.01',
                'action': '_create',
                'did': '1',
                'key': '',
                'msgId': '20170310130900|en_IN',
                'requesterId': '',
                'authToken': superuser_login()["access_token"]
            },
            'WaterConnection': {
                'water': iswaterconn,
                'sewerage': issewerageconn,
                'property': self.property_reponse[0],
                'connectionHolders': ConnHolderDetail,
                'service': isname,
                'roadCuttingArea': None,
                'proposedWaterClosets': None,
                'proposedToilets': None,
                'noOfTaps': None,
                'noOfWaterClosets': None,
                'noOfToilets': None,
                'proposedTaps': None,
                'isMigrated': True,
                'propertyId': self.property_reponse[0]['propertyId'],
                'additionalDetails': {
                    # Your existing fields
                    'initialMeterReading': None,
                    'detailsProvidedBy': '',
                    'isexempted': False,
                    'billingType': 'STANDARD',
                    'billingAmount': 0,
                    'ledgerId': None,
                    'avarageMeterReading': None,
                    'meterMake': None,
                    'compositionFee': None,
                    'userCharges': None,
                    'othersFee': None,
                    'unitUsageType': None,
                    'adhocPenalty': None,
                    'adhocPenaltyComment': None,
                    'adhocPenaltyReason': None,
                    'adhocRebate': None,
                    'adhocRebateComment': None,
                    'adhocRebateReason': None,
                    'estimationFileStoreId': None,
                    'sanctionFileStoreId': None,
                    'estimationLetterDate': None,
                    "connectionCategory": "General",
                    'locality': self.property_reponse[0]['address']['locality']['code'],
                    'groups': waterData.water_data.get('groups'),
                    'pkwsid': waterData.water_data['pkwsid'],
                    'block': waterData.water_data['block'],
                    'propertyno': waterData.water_data['propertyno'],
                    'uidno': waterData.water_data['uidno'],
                    'email': waterData.water_data['email'],
                    'remarks': waterData.water_data['remarks'],
                    'tariff_type': waterData.water_data['tariff_type'],
                    'conn_type': waterData.water_data['conn_type'],
                    'disconn_status': waterData.water_data['disconn_status'],
                    'exempted': waterData.water_data['exempted'],
                    'area_in_sqy': waterData.water_data['area_in_sqy'],
                    # ✅ Added fields from arrear_pending
                    'previous_water': previous_water,
                    'previous_sewerage': previous_sewerage,
                    'total_previous': total_previous,
                    'current_water': current_water,
                    'current_sewerage': current_sewerage,
                    'surcharge': surcharge,
                    'net_amt': net_amt,
                    'gross_amt': gross_amt,
                    'actual_total_arrrear_included_surcharge': actual_total_arrrear_included_surcharge,
                    'watersubusagetype': waterData.water_data.get('watersubusagetype'),
                    'dischargeconnection': waterData.water_data.get('dischargeconnection'),
                    'ownername': waterData.water_data.get('ownername', '').strip(),
                    'guardianname': waterData.water_data.get('guardianname', '').strip(),
                    'category': waterData.water_data.get('category'),
                    'mobile': waterData.water_data.get('mobile'),
                    'arrear': waterData.water_data.get('arrear'),
                    'propertyid': waterData.water_data.get('propertyid'),
                    'guardian': (waterData.water_data.get('guardian') or '').strip(),
                    'nooftaps': waterData.water_data.get('nooftaps'),
                    'ismetered': waterData.water_data.get('ismetered'),
                    'localityname': waterData.water_data.get('localityname'),
                    'last_meter_reading': waterData.water_data.get('last_meter_reading'),
                    'old_meter_reading': waterData.water_data.get('old_meter_reading'),
                    'last_meter_date': waterData.water_data.get('last_meter_date'),
                    'meterid': (waterData.water_data.get('meterid') or '').strip(),
                    'metermake': (waterData.water_data.get('metermake') or '').strip(),
                    'noofseats': (waterData.water_data.get('noofseats') or '').strip(),
                    'receiptdate': (waterData.water_data.get('receiptdate') or '').strip(),
                    'oldconnectionno': (waterData.water_data.get('oldconnectionno') or '').strip(),
                    'mtr_code': (waterData.water_data.get('mtr_code') or '').strip(),
                    'Ledgerid': (waterData.water_data.get('Ledgerid') or '').replace(' ', '') if waterData.water_data.get('Ledgerid') else '',
                    'group id': (waterData.water_data.get('group id') or '').strip(),
                    'digit_localityname': (waterData.water_data.get('digit_localityname') or '').strip(),
                    'digit_locality_code': (waterData.water_data.get('digit_locality_code') or '').strip(),
                    'streetname': (waterData.water_data.get('streetname') or '').strip(),
                    'active': (waterData.water_data.get('active') or '').strip(),
                    'advance': (waterData.water_data.get('advance') or '').strip(),
                    'lastbilldate': (waterData.water_data.get('lastbilldate') or '').strip(),
                    'new_propertyid': (waterData.water_data.get('new_propertyid') or '').strip(),
                    'receiptnumber': (waterData.water_data.get('receiptnumber') or '').strip(),
                    'securitydeposit': (waterData.water_data.get('securitydeposit') or '').strip(),
                    'connectionfee': (waterData.water_data.get('connectionfee') or '').strip(),
                    'others': (waterData.water_data.get('others') or '').strip(),
                    'roadcuttingcharges': (waterData.water_data.get('roadcuttingcharges') or '').strip(),
                    'id_old': (waterData.water_data.get('id_old') or '').strip(),

                },
                'tenantId': self.property_reponse[0]['tenantId'],
                'processInstance': {
                    'action': 'INITIATE'
                }
            }
        }
        response = requests.post(url, headers=headers, data=json.dumps(request_body))


        response_decoded= response.content.decode('utf-8')
        response_decoded = json.loads(response_decoded)
        response_decoded = json.dumps(response_decoded, indent=2)
        request_body_string = json.dumps(request_body)
        # uuid =  waterData.waterData['uuid']


        if(response.status_code==200):
            api_response_dict = response.json()
            application_no = api_response_dict['WaterConnection'][0]['applicationNo']

            url = 'https://mseva.lgpunjab.gov.in/ws-services/wc/_search?tenantId=' + self.property_reponse[0][
                'tenantId'] + '&isConnectionSearch=true&applicationNumber=' + application_no
            request_body_search = {
                'RequestInfo': {
                    'apiId': 'Rainmaker',
                    'ver': '.01',
                    'action': '_create',
                    'did': '1',
                    'key': '',
                    'msgId': '20170310130900|en_IN',
                    'requesterId': '',
                    'authToken': superuser_login()["access_token"]
                }
            }
            search_response = requests.post(url, headers=headers, data=json.dumps(request_body_search))
            search_response = search_response.json()
            connectionNumebr = waterData.water_data['pkwsid']
            oldconnectionno = waterData.water_data.get('oldconnectionno', '')
            print("Water Application Create Request: " + request_body_string)
            print("Water Application Created Succesfully with Response: " + response_decoded)

            # Fetch receipt_date from the database based on pkwsid
            pkwsid = waterData.water_data['pkwsid']
            #cursor_ws.execute("SELECT receipt_date FROM patiala_migrate_records_prod WHERE pkwsid = %s", (pkwsid,))
            cursor_ws.execute("SELECT receiptdate FROM patiala_migrate_records_prod WHERE pkwsid = %s AND isconmig IS NULL",(pkwsid,))
            # cursor_ws.execute("SELECT cd.recpt_dt FROM conn_date cd LEFT JOIN arrear_pending ap ON cd.id_old = ap.id_no LEFT JOIN patiala_migrate_records_prod pmru ON cd.id_old = pmru.oldconnectionno WHERE pmru.pkwsid = %s AND pmru.isconmig IS NULL and cd.conn='W'",(pkwsid,))
            result = cursor_ws.fetchone()

            if result and result[0]:
                receiptdate = result[0]  # This is the string from DB like "03-06-2024" or "    31/01/2024"
                print(f"Fetched receipt_date from DB: {receiptdate}")

                if isinstance(receiptdate, str):
                    receipt_date_str = receiptdate.strip().replace("//", "/")
                    date_formats = ["%d/%m/%Y", "%d-%m-%Y"]

                    for fmt in date_formats:
                        try:
                            receipt_date = datetime.strptime(receipt_date_str, fmt)
                            print(f"Parsed receipt_date using format {fmt}: {receipt_date}")
                            break
                        except ValueError:
                            receipt_date = None

                    if not receipt_date:
                        print(f"Error parsing receipt_date: {receipt_date_str}. No matching format found.")
                        raise ValueError("Invalid date format. Expected DD/MM/YYYY or DD-MM-YYYY")

                elif isinstance(receiptdate, datetime):
                    receipt_date = receiptdate
                    print(f"receipt_date is already a datetime object: {receipt_date}")

                # ✅ Use the parsed datetime object
                epoch_timestamp = int(receipt_date.timestamp() * 1000)
                print(f"Converted epoch timestamp: {epoch_timestamp}")

            else:
                print("No receipt_date found in the database or receipt_date is NULL.")
                epoch_timestamp = None
            print("Water Application Create Request: " + request_body_string)
            print("Water Application Created Succesfully with Response: " + response_decoded)
            # query2="update eg_ws_service set connectioncategory='General', connectiontype='Non Metered', watersource='GROUNDWATER',connectionexecutiondate='"+str({epoch_timestamp})+"' where connection_id='"+api_response_dict['WaterConnection'][0]['id']+"';"
            # Read ismetered from input
            ismetered_val = waterData.water_data.get('ismetered', '').strip().upper()
            connection_id = api_response_dict['WaterConnection'][0]['id']
            disconn_status = (waterData.water_data.get('disconn_status') or '').strip().upper()

            if ismetered_val == 'M':
                connection_type = 'Metered'
            else:
                connection_type = 'Non Metered'

            # Default status
            status_val = "Active"

            # If disconnection status is 'P', mark as Inactive
            if disconn_status == "P":
                status_val = "Inactive"

            # query2 = f"""
            # UPDATE eg_ws_service
            # SET connectioncategory='General',
            #     connectiontype='{connection_type}',
            #     watersource='GROUNDWATER',
            #     connectionexecutiondate='{epoch_timestamp}'
            # WHERE connection_id='{connection_id}';
            # """

            # Safely get meter_id
            meter_id = waterData.water_data.get('meterid')
            meter_id = meter_id.strip() if isinstance(meter_id, str) else None

            # Safely get no_of_taps
            no_of_taps = waterData.water_data.get('nooftaps')
            no_of_taps = str(no_of_taps).strip() if no_of_taps not in (None, '') else None

            # Build update query dynamically
            set_clauses = [
                "connectioncategory='General'",
                f"connectiontype='{connection_type}'",
                "watersource='GROUNDWATER'",
                f"connectionexecutiondate='{epoch_timestamp}'"
            ]

            if meter_id:  # only add if not empty
                set_clauses.append(f"meterid='{meter_id}'")

            if no_of_taps:  # only add if provided
                set_clauses.append(f"nooftaps='{no_of_taps}'")

            set_clause_str = ", ".join(set_clauses)

            query2 = f"""
            UPDATE eg_ws_service 
            SET {set_clause_str}
            WHERE connection_id='{connection_id}';
            """
            
            

            # query="update eg_ws_connection set applicationstatus = 'CONNECTION_ACTIVATED',status ='{status_val}',connectionno = '"+connectionNumebr+"',action ='ACTIVATE_CONNECTION', oldconnectionno = '" + oldconnectionno + "' where applicationno='" +application_no+"' ;"
            query = f"UPDATE eg_ws_connection SET applicationstatus='CONNECTION_ACTIVATED', status='{status_val}', connectionno='{connectionNumebr}', action='ACTIVATE_CONNECTION', oldconnectionno='{oldconnectionno}' WHERE applicationno='{application_no}';"
            # query2= f"""UPDATE eg_ws_service SET connectioncategory='General',connectiontype='{connection_type}',watersource='GROUNDWATER',connectionexecutiondate='{epoch_timestamp}' WHERE connection_id='{connection_id}'; """
            query3="update eg_wf_processinstance_v2 set action='ACTIVATE_CONNECTION', status='e1104474-4188-4cdb-969c-576cabc49155' where businessid='" +application_no+"' ;"
            query=query+query2+query3
            update_query = f"""UPDATE patiala_migrate_records_prod SET water_query_activate = %s, water_upload_status='COMPLETED',water_upload_req = %s,water_upload_res = %s,new_water_application_number=%s,iswatermigrated='True' WHERE pkwsid = %s"""
            cursor_ws.execute(update_query, (query,request_body_string,response_decoded,application_no,str(pkwsid)))
            connection_db_ws.commit()
            # update_demand_query = "UPDATE patiala_migrate_records_prod SET isconnectionmigrated='True' WHERE id_no ='"+connectionNumebr+"'"
            # cursor_ws.execute(update_demand_query)
            print("Water Application Created Succesfully with Application Number: " + application_no)
        else:
            pkwsid = waterData.water_data['pkwsid']
            update_query = f"""UPDATE patiala_migrate_records_prod SET water_upload_req = %s , water_upload_res = %s, water_upload_status ='ERROR' WHERE pkwsid = %s"""
            cursor_ws.execute(update_query, (request_body_string, response_decoded, str(pkwsid)))
        connection_db_ws.commit()



class conn_holder_details:
    def ownerdetails(self, ownername,ownermobileno):
        # Split the string at commas
        split_result = [element.strip() for element in ownername.split(',')]
        surname = set(["sh", "smt", "mr", "mrs"])

        # Remove specified elements from the split array
        cleaned_elements = [item for item in split_result if item.lower() not in surname]

        split_list_of_dicts = [{'sameAsPropertyAddress': False,
                                    'name': cleaned_element,
                                    'mobileNumber':ownermobileno,
                                    'gender': 'MALE',
                                    'relationship': 'Father',
                                    'fatherOrHusbandName': None,
                                    'correspondenceAddress': None,
                                    'ownerType': 'NONE'
                                    } for cleaned_element in cleaned_elements]


        return json.dumps(split_list_of_dicts)
class conn_holder_details:
    def ownerdetails(self, ownername,ownermobileno):
        # Split the string at commas
        split_result = [element.strip() for element in ownername.split(',')]
        surname = set(["sh", "smt", "mr", "mrs"])
        if ownermobileno==None or ownermobileno=='null' or ownermobileno=='':
            ownermobileno="7777777777"
        else:
            ownermobileno=ownermobileno
        # Remove specified elements from the split array
        cleaned_elements = [item for item in split_result if item.lower() not in surname]
        cleaned_elements = [self.remove_symbols(element) for element in cleaned_elements]

        split_list_of_dicts = [{'sameAsPropertyAddress': False,
                                    'name': cleaned_element,
                                    'mobileNumber':ownermobileno,
                                    'gender': 'MALE',
                                    'relationship': 'Father',
                                    'fatherOrHusbandName': None,
                                    'correspondenceAddress': None,
                                    'ownerType': 'NONE'
                                    } for cleaned_element in cleaned_elements]

        return json.dumps(split_list_of_dicts, ensure_ascii=False).encode('utf8').decode('unicode_escape')

    def remove_symbols(self, ownername):
        pattern = re.compile('[^A-Za-z0-9\s]+')

        # Use the pattern to replace symbols with an empty string
        cleaned_string = re.sub(pattern, '', ownername)
        return cleaned_string

