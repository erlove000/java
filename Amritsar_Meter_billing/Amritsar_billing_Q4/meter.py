...
def handle_meter_status():
    ...
    prev_date_epoch = datetime.datetime.strptime("01-01-2025", "%d-%m-%Y")
    prev_date_epoch = int(prev_date_epoch.timestamp() * 1000)
    current_date_epoch = datetime.datetime.strptime("31-03-2025", "%d-%m-%Y")
    ...
    print(prev_date_epoch)
    print(previous_meter_reading)
    print(current_date_epoch)
    print(current_meter_reading)
...
AUTH_TOKEN = 'b82e598b-3dea-4a9a-85c8-e7e24207e84b'
...
#cur.execute("SELECT row_to_json(cd) FROM meter_entry AS cd where response_code='400';")
cur.execute("SELECT row_to_json(cd) FROM meter_billing AS cd where id_no='133997';")
#cur.execute("SELECT row_to_json(cd) FROM meter_billing AS cd where isdead='FALSE' and islocked='FALSE' and response_code is null and prev_rdg >'0'    limit 500 offset 0 ;")
#cur.execute("""SELECT row_to_json(cd) FROM meter_entry AS cd where id_no in ('13895')
#;""")
