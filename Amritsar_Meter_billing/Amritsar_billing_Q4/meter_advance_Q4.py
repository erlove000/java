...
SEARCH_API_TOKEN = '2839bf93-e46d-4b63-98a0-d9fb1928a490'
...
def should_process_connection(connection_no):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            required_periods = ["01/07/2024-30/09/2024", "01/04/2024-30/09/2024"]
...
# Fetch data from DB
#cur.execute("SELECT  row_to_json(cd)  FROM meter_billing_missing as cd where response is null AND isdead = 'FALSE' AND islocked = 'TRUE' and prev_rdg is not null limit 1000 OFFSET 0;")
#cur.execute("SELECT  row_to_json(cd)  FROM meter_billing_missing as cd where response is null AND isdead = 'FALSE' AND islocked = 'FALSE' limit 1000 OFFSET 0;")
#cur.execute("SELECT row_to_json(cd) FROM meter_billing_missing AS cd WHERE id_no = '12519';")
cur.execute("SELECT row_to_json(cd) FROM meter_billing_missing AS cd WHERE id_no in ('108043');")
#cur.execute("SELECT  row_to_json(cd)  FROM meter_billing_missing as cd where response_code='null' limit 250 OFFSET 0;")
